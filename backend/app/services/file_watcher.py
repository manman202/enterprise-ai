...
class DocumentEventHandler(FileSystemEventHandler):
    ...
    def on_created(self, event: FileSystemEvent) -> None:
        ...
        if not event.is_directory and self._should_process(str(event.src_path)):
            logger.info("File watcher: new file detected → %s [dept=%s]", event.src_path, self.department)
    ...
    def on_modified(self, event: FileSystemEvent) -> None:
        ...
        if not event.is_directory and self._should_process(str(event.src_path)):
            logger.info("File watcher: file modified → %s [dept=%s]", event.src_path, self.department)
            self.queue.put_nowait((str(event.src_path), self.department))
...
async def _ingest_worker(queue: asyncio.Queue) -> None:
    ...
    while True:
        ...
        try:
            ...
            logger.error("Ingestion worker: error processing %s — %s", file_path, exc, exc_info=True)
        finally:
            queue.task_done()
